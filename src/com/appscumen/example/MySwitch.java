...
//        mChecked = a.getBoolean(R.styleable.MySwitch_checked, true);
        ...
        Log.d(TAG, "mTrackDrawable.getBounds()=" + mTrackDrawable.getBounds());
...
//        setChecked(mChecked);
    ...
    private Layout makeLayout(CharSequence text) {
        return new StaticLayout(text, mTextPaint,
                ...
                Layout.Alignment.ALIGN_OPPOSITE, 1.f, 0, true);
    }
    ...
    */
    ...
    public void setChecked(boolean checked) {
        ...
        float endPostion = lc ? getThumbScrollRange() : 0;
        startThumbAnimate(endPostion,checked);
    }
    
    private void startThumbAnimate(float endPostion, final boolean checked) {
		
		float startPos = mThumbPosition;
		float endPos = endPostion;
		int time = 200;//ms
		float dePos = endPos - startPos;
		final float speed = dePos/time;
//		final float total = getThumbScrollRange();
		if(speed == 0) {
			MySwitch.super.setChecked(checked);
			MySwitch.this.invalidate();
			return;
		}
		MySwitch.this.post(new AnimationTransRunnable(checked, speed));
	}
	
	private class AnimationTransRunnable implements Runnable {

		private final float total = getThumbScrollRange();
		private boolean checked;
		private float speed;
		private static final int SIXTY_FPS_INTERVAL = 1000 / 30;
		
		public AnimationTransRunnable(boolean checked, float speed) {
...
/*    @Override
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int switchWidth = Math.max(maxTextWidth
				+ mThumbWidth + mTrackTextPadding * 2
...
//    				(mTrackPaddingRect.left + mTrackPaddingRect.right)/2);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		Log.d(TAG, "onLayout()-left=" + left + ",top="+top+",right="+right+",bottom="+bottom+",changed="+changed);
        super.onLayout(changed, left, top, right, bottom);
...
//        mSwitchTop = mSwitchBottom-mSwitchHeight;
        mSwitchTop = getPaddingTop();
        mSwitchRight = getWidth()-getPaddingRight();
...
//                    	int l = (tempBitmap.getPixel(x,y) & 0xFF000000);
//                    	Log.i(TAG, "l"+l);
            	        maskBitmap.setPixel(x, y, (tempBitmap.getPixel(x,y) & 0xFF000000));
                    }
                }*/
                maskBitmap = tempBitmap.copy(tempBitmap.getConfig(), true);
            }
...
//            final int switchVerticalMid = (switchInnerTop + switchInnerBottom) / 2;
            ...
            if(mOnLayout != null) {
            	backingLayer.save();
            	backingLayer.translate(mTrackPaddingRect.left + mTrackTextPadding + (maxTextWidth-mOnLayout.getWidth())/2, midHeight-mOnLayout.getHeight()/2);
            ...
            if(mOffLayout != null) {
            	backingLayer.save();
            	backingLayer.translate(width/2 + mThumbWidth/2 + (maxTextWidth-mOffLayout.getWidth())/2 + mTrackTextPadding, midHeight-mOffLayout.getHeight()/2);
...
//        	int thumbBoundR = switchInnerLeft + thumbPos + mThumbWidth -mThumbExtraMovement;// - mThPad.right
        	...
        	int thumbBoundR = thumbBoundL + mThumbWidth;
...
//        		backingLayer.translate(-thumbRange+thumbPos,0);
        		backingLayer.translate(-(pushBitmap.getWidth()/2-thumbBoundL-mThumbWidth/2),0);
